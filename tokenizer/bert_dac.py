...
class BertDacTokenizer:
    ...
    def __init__(self, audio_codec: nn.Module) -> None:
        ...
        # self.tok = AutoTokenizer.from_pretrained("hon9kon9ize/bert-base-cantonese")
        # self.tok = AutoTokenizer.from_pretrained("bert-base-chinese")

        # Audio encoder attributes
        self.codebook_size = audio_codec.codec.codebook_size  # E.g., 1024 (from the pretrained 44khz model)
    ...
    def captions_to_ids(
        ...
        for caption in captions:
            ...
            # Append special IDs
            ids = [self.tok.cls_token_id] + ids + [self.tok.sep_token_id] # cls_token_id(101) & sep_token_id(102)
    ...
    def ids_to_audio_codes(self, ids: torch.LongTensor) -> torch.LongTensor:
        ...
        t: time_step after ignore 'eoa'
        t*q must smaller than max_new_ids(2048)
        ...
        for b in range(B):
            ...
            for t in range(T):
                ...
                if match: # ignore the 'eoa' (which is no sound)

                    q = int(match.groups()[0]) # get n_quantizers
                    id = int(match.groups()[1]) # get codebook position

                    if q == 0:  # for safety reset the buffer again
                        buffer = []

                    buffer.append(id)

                    if q == self.n_quantizers - 1:  # record (2, 1) id to codes
                        if len(buffer) == self.n_quantizers:
                            codes.append(buffer)
                            buffer = []

            codes = torch.LongTensor(codes) # number of id when q == 0
            # print('original: ', codes.shape)
            codes = rearrange(codes, 't q -> q t')  # shape: (q, t)
            # print('after arrange: ', codes.shape)
